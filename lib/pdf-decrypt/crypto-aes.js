...
/**
 * AES-256-CBC encrypt with PKCS#7 padding (standard Web Crypto)
 *
 * @param {Uint8Array} data - Plaintext bytes
 * @param {Uint8Array} key - 32-byte AES-256 key
 * @param {Uint8Array} iv - 16-byte initialization vector
 * @returns {Promise<Uint8Array>} - Ciphertext (multiple of 16 bytes)
 */
...
/**
 * AES-256-CBC encrypt without padding (for UE/OE keys — exactly 32 bytes)
 * Uses standard CBC encrypt and truncates the padding block.
 *
 * @param {Uint8Array} data - Plaintext bytes (must be multiple of 16 bytes)
 * @param {Uint8Array} key - 32-byte AES-256 key
 * @param {Uint8Array} iv - 16-byte initialization vector
 * @returns {Promise<Uint8Array>} - Ciphertext (same length as data)
 */
export async function aes256CbcEncryptNoPad(data, key, iv) {
  const encrypted = await aes256CbcEncrypt(data, key, iv);
  return encrypted.slice(0, data.length);
}

/**
 * AES-256-ECB encrypt a single 16-byte block (for Perms encryption)
 *
 * @param {Uint8Array} block - 16-byte plaintext block
 * @param {Uint8Array} key - 32-byte AES-256 key
 * @returns {Promise<Uint8Array>} - 16-byte ciphertext block
 */
export async function aes256EcbEncryptBlock(block, key) {
  const zeroIV = new Uint8Array(16);
  return await aes256CbcEncryptNoPad(block, key, zeroIV);
}
